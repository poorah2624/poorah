			...
			<div class="modern-cart-container">
					...
					<c:forEach var="c" items="${cartItems}" varStatus="status">
						<c:choose>
							...
							<c:when test="${c.isCustom}">
								<div class="custom-cart-card">
									<div class="custom-img">
										<img src="${c.customImage}" alt="Custom Design"/> 
									...
									<div class="custom-details">
										<div>
											...
											<div class="meta-info">
												...
												<span><b>Color:</b> ${c.color}</span>
												<span><b>Type:</b> ${c.tshirtType}</span>
												<span><b>Gender:</b> ${c.gender}</span>
											</div>
											<c:if test="${not empty c.customNote}">
												<p style="font-size:12px; color:#666; margin-top:5px;">
													<i class="glyphicon glyphicon-comment"></i> <b>Note:</b> ${c.customNote}
												</p>
											</c:if>
										</div>
										...
										<div class="action-row">
											...
											<a href="/DeleteCart/${c.cartId}" class="delete-btn" title="Remove Item">🗑</a>
										</div>
									</div>
								</div>
							</c:when>
							...
							<c:otherwise>
								<div class="cart-card">
									<div class="cart-img">
										<img src="${fn:split(c.item.itemImage, ',')[0]}" alt="${c.item.itemName}"/>
									</div>
									...
									<div class="cart-details">
										<div>
											...
											<div class="meta-info">
												<c:if test="${c.item.category.categoryName == 'Men' || c.item.category.categoryName == 'Women'}">
													<span><b>Size:</b> ${c.size}</span>
												</c:if>
											...
											<div class="price-wrapper">
												...
												<span class="old-price">₹ ${c.item.itemPrice}</span> 
												<span class="discount">${c.item.discount}% OFF</span>
											</div>
										</div>

										<div class="action-row">
											<form action="/UpdateQuantity" method="post" style="display:flex; align-items:center;">
												<input type="hidden" name="itemId" value="${c.item.itemId}">
												<input type="hidden" name="userEmail" value="${c.user.userEmail}">
				...
				<div class="cart-summary-card">
					...
					<div class="summary-row">
						...
						<span style="color: ${deliveryCharge == 0 ? 'green' : '#666'}; font-weight: ${deliveryCharge == 0 ? '600' : 'normal'};">
							<c:choose>
					...
					<c:if test="${grandTotal < 500}">
						<div class="delivery-tip" style="background: #fff3cd; color: #856404;">
							Add <b>₹${500 - grandTotal}</b> more to get FREE Delivery 🚚
						</div>
					</c:if>
					<c:if test="${grandTotal >= 500}">
						<div class="delivery-tip" style="background: #d4edda; color: #155724;">
					...
					<div class="checkout-actions">
						...
						<a href="/products2" class="btn-continue">
