...
public class ShiprocketService {
	...
	@Autowired
	private OrderRepo orderRepo; // Inject repository to save the ID
	
	private String cachedToken;
	private long tokenTime;
	
	public String getToken() {
		if (cachedToken != null && (System.currentTimeMillis() - tokenTime) < 50 * 60 * 1000) {
			return cachedToken;
		}
        ...
        try {
            ...
            for (Order_item item : order.getItems()) {
                ...
                if (item.getIsCustom() != null && item.getIsCustom()) {
                    ...
                    itemJson.put("name", item.getItem().getItemName());
                }
            ...
            body.put("order_date", order.getOrderDate().toString());
            body.put("pickup_location", "Home");
            body.put("billing_customer_name", order.getUser().getUserName());
            body.put("billing_last_name", "");
            
            body.put("billing_address", order.getAddress().getHouseNo() + ", " +
                    order.getAddress().getArea() + ", " + order.getAddress().getLandmark());
            ...
            body.put("billing_email", order.getUser().getUserEmail());
            body.put("billing_phone", order.getAddress().getMobile());
            ...
            ResponseEntity<String> response = restTemplate.postForEntity(
                            "https://apiv2.shiprocket.in/v1/external/orders/create/adhoc", request, String.class);

            System.out.println("SHIPROCKET RESPONSE:");
            System.out.println(response.getBody());

            // 🔥 EXTRACT AND SAVE SHIPROCKET ORDER ID
            JSONObject responseJson = new JSONObject(response.getBody());
            if (responseJson.has("order_id")) {
                Long shiprocketId = responseJson.getLong("order_id");
                order.setShiprocketOrderId(shiprocketId);
                orderRepo.save(order); // Database me sync metadata update hua
                System.out.println("✅ Saved Shiprocket Numeric ID to DB: " + shiprocketId);
            }
	...
	public void cancelOrderInShiprocket(Orders order) {
	    try {
	        if (order.getShiprocketOrderId() == null) {
	            System.out.println("⚠️ Order Shiprocket me create hi nahi hua tha (Numeric ID Missing).");
	            return;
	        }
	        ...
	        // Numeric array setup for standard cancel path
	        JSONObject payload = new JSONObject();
	        JSONArray idsArray = new JSONArray();
	        idsArray.put(order.getShiprocketOrderId()); 
	        ...
	        System.out.println("❌ Shiprocket Cancellation Failed: " + e.getMessage());
	    }
	}
}